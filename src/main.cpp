...
#if defined(NDEBUG)
# error "Wisecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb5ccabcb5a9934881a5a42bed3bfca6ecf81947013f695ed66d3c773ffeeb213");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Wisecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Wisecoin Signed Message:\n";
...
{
    // Wisecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Wisecoin: 10k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Wisecoin: 1 day
static const int64 nTargetSpacing = 2.5 * 60; // Wisecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Wisecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Wisecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc2b3e998b5724d361205b3ff26a8131c882505260897b6bff6f3bb9f93428924");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 03/Mar/2018 Show of Force as Students’ Grief Inspires Huge Gun Protests";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 10 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040d9ae3c01e4c3f17236826951ffa1b0f12d96758094420c63f3b0eacba3bdc9cb431dfc69c9480f0d262bb8087d1860674c46b50e87c4e954cfb1108bca474d9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1521989423;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085785302;

        if (fTestNet)
        {
            block.nTime    = 1521989374;
            block.nNonce   = 385900852;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x0840db367ccfd0d4916360a46b3581768e105909479856ec285a0575451a3f48"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb5, 0xd1 }; // Wisecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WisecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WisecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WisecoinMiner(CWallet *pwallet)
{
    printf("WisecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("wisecoin-miner");
    ...
    try { loop {
        ...
        printf("Running WisecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("WisecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WisecoinMiner, pwallet));
}
